...
完整的小程序活动文本仍可生成原有的删减分享版。“试算”旁的“只保留报名名单”图标按钮会移除标题、活动说明、场地和报名链接，只留下原顺序的编号报名行；中间卡片随即恢复可排序的接龙列表。

带方括号等级的报名行以 🌸 标记确定性别：有标记为女，无标记为男。方括号中的等级不参与计算；已匹配成员使用数据库等级，陌生人或随行人员显示等级下拉框，由管理员手动设置。手动编辑只影响本次接龙，不改写左侧原文或成员数据库。编号随行人员和重复报名保留独立的性别、水平及归属，试算开关状态不受整理操作影响。GitHub 只读版同样支持本地整理与等级选择。

使用 `npm run test:special-roster` 验证 33 人示例、原文保留、等级来源、性别统计、排序、随行人员和普通接龙回归。

## 群收款规则

男女混合时，女性每人比男性少付 3.50 元，不再设置 25 元封顶。设总费用为 T、付款人数为 N、女性人数为 F，则男性单价为 `(T + 3.50 × F) / N` 向上取整到分，女性单价为男性单价减 3.50 元。所有收款金额、分组合计、复制文本和费用预估统一显示两位小数。

只有一种性别时按人数均摊并向上取整到分；性别不详沿用普通单价。极低费用下女性单价最低为 0 元，剩余费用由男性均摊，避免负数收款。随行人员按自己的性别计算，再按归属人汇总。已保存的历史账单不自动重算。

例如总费用 115 元、3 男 1 女：男性每人 29.63 元，女性 26.13 元，合计 115.02 元（向上取整产生 0.02 元差额）。使用 `npm run test:payment` 验证生产计算函数及显示。
